...
import { getLeccionOptimizaBySlug, getNextLeccionOptimiza, getPrevLeccionOptimiza, leccionesOptimiza } from '@/lib/optimiza-data';
import CursoOptimizaSidebar from '@/components/CursoOptimizaSidebar';

export async function generateStaticParams() {
  return leccionesOptimiza.map((leccion) => ({
    slug: leccion.id,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const leccion = getLeccionOptimizaBySlug(slug);
  ...
  return {
    title: `${leccion.titulo} - Optimiza Tus Compras en China`,
    description: leccion.descripcion,
  };
}

export default async function LeccionOptimizaPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const leccion = getLeccionOptimizaBySlug(slug);
  ...
  const nextLeccion = getNextLeccionOptimiza(leccion.id);
  const prevLeccion = getPrevLeccionOptimiza(leccion.id);
    ...
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="grid lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="space-y-8">
          ...
          <div>
            <div className="flex items-center gap-3 text-sm text-gray-500 mb-4">
              <Link href="/curso-optimiza-compras" className="hover:text-gray-900">
            ...
            <div className="flex items-center gap-6 text-sm text-gray-600">
              ...
              <span className="flex items-center gap-1.5">
                <span className="text-lg">🎯</span>
                Lección {leccion.numero} de {leccionesOptimiza.length}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-8 border-t border-gray-200">
            <div>
              {prevLeccion ? (
                <Link
                  href={`/curso-optimiza-compras/leccion/${prevLeccion.id}`}
            ...
            <div>
              {nextLeccion ? (
                <Link
                  href={`/curso-optimiza-compras/leccion/${nextLeccion.id}`}
                  className="inline-flex items-center gap-2 text-gray-600 hover:text-gray-900 transition-colors group"
                >
        ...
        <div className="lg:block hidden">
          <CursoOptimizaSidebar currentLeccionId={leccion.id} />
        </div>
      </div>
    </div>
  );
