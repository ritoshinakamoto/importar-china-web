...
interface CursoProveedorSidebarProps {
  currentLeccionId: string;
}

export default function CursoProveedorSidebar({ currentLeccionId }: CursoProveedorSidebarProps) {
  const leccionesPorModulo = modulosProveedor.map(modulo => ({
    ...modulo,
    lecciones: leccionesProveedor.filter(l => l.modulo === modulo.nombre)
  }));

  const currentIndex = leccionesProveedor.findIndex(l => l.id === currentLeccionId);
  const progress = Math.round(((currentIndex + 1) / leccionesProveedor.length) * 100);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Lección {currentIndex + 1} de {leccionesProveedor.length}
        </p>
      </div>
          ...
          <div key={modulo.id}>
            ...
            <div className="space-y-1">
              {modulo.lecciones.map((leccion) => {
                ...
                const isPast = leccionesProveedor.findIndex(l => l.id === leccion.id) < currentIndex;
