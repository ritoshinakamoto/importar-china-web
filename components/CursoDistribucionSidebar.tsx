...
export default function CursoDistribucionSidebar({
  currentSlug,
}: {
  currentSlug: string;
}) {
  ...
  const progress = Math.round(((currentIndex + 1) / todasLecciones.length) * 100);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Lección {currentIndex + 1} de {todasLecciones.length}
        </p>
      </div>
          ...
          <div key={idx}>
            ...
            <div className="space-y-1">
              {modulo.lecciones.map((leccion, leccionIdx) => {
                ...
                const leccionIndex = modulos
                  .slice(0, idx)
                  .reduce((acc, m) => acc + m.lecciones.length, 0) + leccionIdx;
