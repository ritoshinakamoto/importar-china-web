...
interface CursoMetricasSidebarProps {
  currentLeccionId: string;
}

export default function CursoMetricasSidebar({ currentLeccionId }: CursoMetricasSidebarProps) {
  const leccionesPorModulo = modulosMetricas.map(modulo => ({
    ...modulo,
    lecciones: leccionesMetricas.filter(l => l.modulo === modulo.nombre)
  }));

  const currentIndex = leccionesMetricas.findIndex(l => l.id === currentLeccionId);
  const progress = Math.round(((currentIndex + 1) / leccionesMetricas.length) * 100);
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-xs text-gray-500 mt-2">
          Lección {currentIndex + 1} de {leccionesMetricas.length}
        </p>
      </div>
          ...
          <div key={modulo.id}>
            ...
            <div className="space-y-1">
              {modulo.lecciones.map((leccion) => {
                ...
                const isPast = leccionesMetricas.findIndex(l => l.id === leccion.id) < currentIndex;
